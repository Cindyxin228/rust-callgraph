...
//获取cargo包的元数据，用于获取目标包的信息，包括目标包的名称、路径、目标等信息，确定要分析哪些源文件和目标文件
...
//把自己作为wrapper，参与到每一个文件的编译过程
//用更生活化的比喻：
//第一次运行就像你告诉助手："我要分析这个项目"
//助手说："好的，我来帮你处理，但是我需要先编译整个项目"
//然后助手（把自己作为wrapper）会参与到每一个文件的编译过程
//对于第三方库的代码，助手说："这个不需要分析，正常编译就行"
//对于你自己的代码，助手说："这个需要分析，我要用特殊工具处理"
//这样设计的目的是为了能够精确地控制编译过程，只对真正需要分析的代码进行处理，而对其他代码保持正常的编译流程。
fn main() {
...
//将cargo_metadata::Target转换为TargetKind，判断目标类型，是库还是二进制文件，还是未知
...
//核心功能，编译项目，并收集调用关系，并生成调用图，dot文件，json文件，svg文件
fn in_cargo_callgraph() {
    ...
    //获取包的元数据
    //收集所有编译目标
    //按照类型排序：库(Library) 优先于二进制文件(Bin)，确保依赖关系正确
    ...
    for target in targets {
        ...
        //拦截的主要作用是：
        //性能优化：只分析用户关心的代码，而不是所有依赖
        //准确性：保持原有的编译配置和特性
        //灵活性：可以同时使用 cargo 的编译选项和 callgraph 的分析选项
        //就像一个"智能过滤器"：
        //对依赖包说："你们走普通编译通道"
        //对目标代码说："你来这边，我要仔细分析你"
        //同时又能保持 cargo 生态系统的所有优势
        //  Cargo -> cargo-callgraph(作为wrapper) -> rustc
        ...
        //超时处理是一个安全保障机制：
        //保护系统资源：防止无限占用 CPU/内存
        //提升用户体验：给出明确的错误提示
        //确保可靠性：异常情况下也能优雅退出
        //便于调试：知道是超时导致的失败，而不是其他原因
